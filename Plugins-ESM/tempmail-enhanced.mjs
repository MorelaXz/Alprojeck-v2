  ...
  async fetchWithRetry(url, options = {}, maxRetries = CONFIG.api.maxRetries) {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        lastError = error;
        console.error(`[HTTP] Attempt ${attempt + 1}/${maxRetries} failed: ${error.message}`);
      ...
      if (messageCount === 1) {
        ...
        const messagePromises = links.map(async (link) => {
          try {
            ...
            console.error(`[INBOX] Failed to fetch message ${link}:`, error.message);
            return null;
          }
        });

        const results = await Promise.all(messagePromises);
        messages.push(...results.filter(msg => msg !== null));
      }
...
function drawHeader(ctx, icon, title, subtitle) {
...
function drawEmailBox(ctx, email, yPosition = 145) {
  const { colors, fonts } = CONFIG.canvas;
...
async function createGenerateCard(data) {
...
async function createInboxCard(data) {
  const { canvas, ctx } = createBaseCanvas();
  const { colors, fonts } = CONFIG.canvas;
...
async function createValidateCard(data) {
  const { canvas, ctx } = createBaseCanvas();
  const { colors, fonts } = CONFIG.canvas;
...
async function createTempMailCard(data, type) {
  try {
    switch (type) {
      case "generate":
